...
class Model(nn.Module):
    ...
    #              single_step_output_One = 0, input_len_seg = 0, positionalE = False, modified = True, RIN=False):
        ...
        self.trend = nn.Linear(configs.seq_len, configs.seq_len)
        self.trend_dec = nn.Linear(configs.seq_len, configs.pred_len)
        self.blocks1 = EncoderTree(
            ...
            INN =  configs.modified)

        if configs.num_stacks == 2: # we only implement two stacks at most.
        ...
        self.stacks = configs.num_stacks
        ...
        self.pe_hidden_size = configs.input_dim
        ...
        ### RIN Parameters ###
        if self.RIN:
            self.affine_weight = nn.Parameter(torch.ones(1, 1, configs.input_dim))
            self.affine_bias = nn.Parameter(torch.zeros(1, 1, configs.input_dim))
            self.affine_weight2 = nn.Parameter(torch.ones(1, 1, configs.input_dim))
            self.affine_bias2 = nn.Parameter(torch.zeros(1, 1, configs.input_dim))
